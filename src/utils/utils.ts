...
import { getUpcomingConfirmedAppointmentNameAndDateTime } from "./axios/common/getAppointment";
import { peopleData } from "./Constants";
import { Artist, DateTimeDto, SubService } from "./types";
...
export const updateBookedTimes = async (): Promise<Artist[]> => {
  ...
  const appointments = await getUpcomingConfirmedAppointmentNameAndDateTime();

  // Create a copy of the peopleData array to avoid mutating the original data
  const updatedPeopleData = [...peopleData];

  // Iterate over the appointments and update the dateTimeDto for the matching artist
